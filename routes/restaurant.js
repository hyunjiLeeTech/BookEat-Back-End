...
router.route("/reserve").post(async (req, res) => {
  ...
  if (
    !(await isTableAvaliableAtTime(table, new Date(obj.dateTime), eatingTime))
  ) {
    res.json({
      errcode: 3,
...
router.route("/reservationsofpast14days").get(async (req, res) => {
  var u = {
    _id: "5efa8fe8dd9918ba08ac9ae0",
    userType: 3,
    restaurantId: "5efa8fc9dd9918ba08ac9ade",
  }; //FIXME: for debug restaurant maanger
  if (u.userType === 2) {
...
router.route("/:id").post((req, res) => {
  var obj = {
    ...
    businessNum: req.body.businessnumber,
    description: req.body.description,
    status: 2, // completed restaurant profile
